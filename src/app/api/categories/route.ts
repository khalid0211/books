...
export async function GET(req: Request) {
  ...
  return NextResponse.json(await prisma.category.findMany({ orderBy: { name: "asc" }, select: { id: true, name: true, _count: { select: { books: true } } } }));
}
...
async function save(req: Request, update: boolean) {
  ...
  if (update && (!Number.isSafeInteger(body.id) || body.id < 1)) return NextResponse.json({ error: "Select a valid category." }, { status: 400 });
  const data = { name, nameKey: name.toLowerCase() };
  try {
    const category = update ? await prisma.category.update({ where: { id: body.id }, data }) : await prisma.category.create({ data });
    return NextResponse.json(category, { status: update ? 200 : 201 });
  } catch (e) {
    if (e instanceof Prisma.PrismaClientKnownRequestError) {
      if (e.code === "P2002") return NextResponse.json({ error: "That category already exists." }, { status: 409 });
      if (e.code === "P2025") return NextResponse.json({ error: "Category no longer exists." }, { status: 404 });
    }
